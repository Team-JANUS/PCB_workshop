...
bool isPressedJump() {
  if(enableled=1){
    digitalWrite(JUMP_LED,(!digitalRead(JUMP_BUTTON)?HIGH:LOW));
  }
...
bool isPressedDuck() {
  if(enableled=1){
  digitalWrite(DUCK_LED,(!digitalRead(DUCK_BUTTON)?HIGH:LOW));
  }
...
void gameLoop(uint16_t &hiScore) {
  ...
  while(1) {
    ...
    while(1) {
      ...
      //game over
      if(gameOver && !marioThemePlayed) {
        playMarioTheme();
        marioThemePlayed = true;
