...
private:
 ...
 void reinit() {
    ...
    sendc(inverted ? 0xA7 : 0xA6);

    // Rotate display 180 degrees by modifying segment and common outputs
    sendc(0xA0 | 0x01);  // Set Segment Re-map (A0 normal, A1 180 degrees)
    sendc(0xC8);         // Set COM Output Scan Direction (C0 normal, C8 180 degrees)
}
